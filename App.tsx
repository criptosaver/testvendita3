...
const App: React.FC = () => {
  ...
  const [data, setData] = useState<GuideData>(() => {
    const saved = localStorage.getItem('guide_data_v1');
  ...
  useEffect(() => {
    localStorage.setItem('guide_data_v1', JSON.stringify(data));
  }, [data]);
  ...
  // Helper per WhatsApp (solo numeri)
  const cleanPhoneForWhatsApp = (phone: string) => phone.replace(/\D/g, '');
  // Helper per chiamate (mantieni + ma rimuovi spazi)
  const cleanPhoneForCall = (phone: string) => phone.replace(/[^\d+]/g, '');
    ...
    return parts.map((part, i) => {
      ...
      if (part.trim().match(/^\+39/)) {
         return (
          <a 
            key={i}
            href={`tel:${cleanPhoneForCall(part)}`}
    ...
    if (section.id === 'taxi' && !isAdmin) {
      ...
      return (
        <div className="px-6 pb-8 pt-2 space-y-5">
           ...
           {/* Call Button */}
           <a 
             href={`tel:${cleanPhoneForCall(taxiNumber)}`}
           ...
           >
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <div className="font-black text-lg">Taxi Service</div>
  ...
  return (
    <div className="min-h-screen pb-10 max-w-2xl mx-auto bg-stone-50 text-stone-800 selection:bg-amber-100 font-sans border-x border-stone-200 relative shadow-2xl">
      {/* DEMO BANNER */}
      <div className="sticky top-0 z-[100] bg-stone-900/95 backdrop-blur-md text-white py-3 px-6 shadow-xl flex items-center justify-between border-b border-stone-800">
        <div className="flex items-center gap-3">
            <div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse shadow-[0_0_10px_rgba(52,211,153,0.5)]" />
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-300">Demo Preview</span>
        </div>
        <a 
          href="https://wa.me/393356626901" 
          target="_blank" 
          rel="noopener noreferrer"
          className="text-[10px] font-bold bg-white text-stone-950 hover:bg-emerald-400 transition-all px-3 py-1.5 rounded-lg border border-transparent uppercase tracking-wider flex items-center gap-1.5 active:scale-95"
        >
      ...
      <div className="relative h-[24rem] overflow-hidden border-b border-stone-200 shadow-sm">
        <img 
          src="https://images.unsplash.com/photo-1566665797739-1674de7a421a?auto=format&fit=crop&q=80&w=2074" 
          alt="B&B Bedroom" 
          ...
          {Object.values(Language).map(l => (
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center overflow-hidden border-2 transition-all ${
                lang === l 
                  ? 'border-white shadow-lg scale-110 ring-2 ring-amber-50' 
        ...
        <div className="absolute bottom-10 left-8 right-8">
          <div className="flex items-center gap-2 mb-3">
             <span className="px-3 py-1 bg-amber-100 text-amber-800 border border-amber-200 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">Welcome Guide</span>
          </div>
          <h1 className="text-4xl sm:text-5xl font-black tracking-tighter text-stone-900 mb-2">{data.propertyName}</h1>
      ...
      <div className="px-6 py-10 space-y-10">
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <a
            href={`https://wa.me/${cleanPhoneForWhatsApp(data.contacts.whatsapp)}`}
            target="_blank"
            rel="noopener noreferrer"
        ...
        <div className="bg-white rounded-[3.5rem] p-10 border border-stone-200 shadow-xl relative overflow-hidden group">
          ...
          <div className="flex flex-col sm:flex-row gap-5 relative">
            <a 
              href={`tel:${cleanPhoneForCall(data.contacts.phone)}`}
          ...
          <div className="mt-6 grid grid-cols-3 gap-2">
            {[
              ...
              <a
                key={i}
                href={`https://wa.me/${cleanPhoneForWhatsApp(data.contacts.whatsapp)}?text=${encodeURIComponent(msg.label)}`}
                target="_blank"
                rel="noopener noreferrer"
      ...
      <div className="flex flex-col items-center justify-center pb-10 opacity-30 hover:opacity-100 transition-opacity">
        <button
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl border border-stone-200 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-stone-500 text-sm mb-6">Enter password to edit guide (Pass: admin).</p>
